...
bool CanGo(int x, int y, int dir) {
    if (dir == 4) {      // 좌측위
        ...
        if (!wall[left_x][left_y][1] && !wall[left_x][left_y][2]) return true;
    }
    else if (dir == 5) {     // 우측위
        int right_x = x + dx[2];
        int right_y = y + dy[2];

        if (!wall[right_x][right_y][0] && !wall[right_x][right_y][1]) return true;
    }
    else if (dir == 6) {     // 우측아래
        int right_x = x + dx[3];
        int right_y = y + dy[3];

        if (!wall[right_x][right_y][1] && !wall[right_x][right_y][2]) return true;
    }
    else if (dir == 7) {      // 좌측아래
        int left_x = x + dx[3];
        int left_y = y + dy[3];

        if (!wall[left_x][left_y][0] && !wall[left_x][left_y][1]) return true;
    }
...
void Spread(int x, int y, int d) {
    ...
    while(!q.empty()) {
        ...
        for(int dir : dirs) {
            ...
            else if(dir >= 4 && CanGo(cur.x, cur.y, dir)) {
                q.push({nx, ny, cur.d - 1});
                vis[nx][ny] = true;
                A[nx][ny] += (cur.d - 1);
            }
